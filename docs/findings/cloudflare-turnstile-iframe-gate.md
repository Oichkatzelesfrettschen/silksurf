...
The API's `reject` handler compares the received message's `reason` with the string `unsupported_browser` and calls the widget's unsupported callback. The challenge child supplies that reason. Its browser-admission predicate remains opaque in the live evidence. The fresh `forceFail` run exposes a separate runtime incompatibility; the `let` diagnostic does not yet identify the dynamically evaluated source or the browser property that selects it.
...
Cloudflare's exact browser-admission predicate remains unobserved. The latest SilkSurf run reaches a child-script runtime error and `forceFail`, while an earlier run reached `reject reason=unsupported_browser`; the two outcomes remain separate observations. The next decisive trace captures the dynamically evaluated source and the challenge child's browser-property reads in SilkSurf and Chromium. Child input routing remains unverified because SilkSurf does not yet paint the checkbox.
