...
Chromium's fresh request capture also includes `Sec-CH-UA: "Chromium";v="153", "Not_A Brand";v="8"`, `Sec-CH-UA-Mobile: ?0`, and `Sec-CH-UA-Platform: "Linux"`. Its parent document exposes `navigator.userAgentData` with those brands, `mobile=false`, `platform=Linux`, architecture `x86`, bitness `64`, and full version `153.0.8010.36`. SilkSurf previously exposed no `navigator.userAgentData` and sent none of the three low-entropy request headers. SilkSurf now exposes its own truthful profile in HTTPS and loopback contexts and sends matching low-entropy headers on HTTPS requests; caller-provided request values take precedence. A fresh SilkSurf A/B run with Chromium's three low-entropy request values still receives `reject reason=unsupported_browser`, so the missing Client Hints are a real compatibility gap and are not the sole rejection cause. The challenge's browser-admission predicate remains unobserved.
