...
- `chatgpt-live-acceptance`: the native run executes the 69,750-byte bootstrap,
  ...
  path now flattens ordinary `display:contents` wrappers in block, flex, and
  equal-`fr` grid flow; inline formatting, SVG box-generation exceptions,
  and computed sizing still need live geometry evidence. Login, prompt
